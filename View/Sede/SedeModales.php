...
$ingreso = Http::permisos($permisos->getId(), $permisos->getIdTipo(), "SEDE");
...
$sede = new Sede( ' codigosede ' , $llave_Primaria_Contructor);
if ($id == 1 && $permisos)
{
?>
    <div class="carga_Documento">
        <div class="contenido">  
            ...
            <br><br>
            <label style="font-size: 1em; " >Tabla Sede</label>  
            <label style="font-size: 1em; " id="aviso" class="aviso" ></label> 
            <label style="font-size: 1em; " id="aviso2" class="aviso" ><?= $sede->getCod() ?></label> 
        </div> 
        <div>
            <fieldset>
        ...
        <div>
            <fieldset>
                ...
                <select  required name='id_departamento' id="id_departamento">
                    <?= Select::listaopciones( 2 , $sede->getId_departamento() , "select id , nom_departamento from departamento ;" )?>
                </select>   
            </fieldset>
        </div>
        <div>        
            <input type="hidden" value="<?= $sede->getCod() ?>" name="id" id="id">
    ...
    <div class="carga_Documento">
        <div class="contenido">  
            <div class="where_title where_modal" style="width: 100%; height: auto; margin-left: 0px;">
                ...
                <lablel>
                    Se realizara la accion "<?= $accion ?>" al centro de Formación o Dirección <?=$llave_Primaria?> cargado en el modulo de la Dirección de Formación Profesional.
                </label>
            </div><br><br>
...
?>
    <div class="carga_Documento">
        ...
        <div class="nuevaseccion" >
            <fieldset>
                <section>
                    <h3>CODIGO DE CENTRO: </h3> 
                    <p> <?= $sede->getCod() ?></p>
                </section>
                <section>
                    <h3>NOMBRE DE CENTRO: </h3> 
                    <p> <?= $sede->getNombre() ?></p>
                </section>
                <section>
                    <h3>CODIGO DE REGIONAL: </h3> 
                    <p> <?= $sede->getId_departamento() ?></p>
                </section>
                <section>
                    <h3>NOMBRE DE REGIONAL: </h3> 
                    <p> <?= $sede->getDepartamento() ?></p>
                </section>
            </fieldset>
        </div>
    </div>

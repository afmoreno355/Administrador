...
if ($ingreso === false && $permisos->getIdTipo() !== "SA") {
    ...
    if ($bucarPalabraClave != "") {
        $filtro .= " ( codigocontratos like '%" . strtoupper($bucarPalabraClave) . "%' or  nombrecontratos like '%" . strtoupper($bucarPalabraClave) . "%' )";
    }
    ...
    <div class="botonMenu" style="font-weight: bolder; font-size: 2em; ">
        <button type='button' id='button' class="ele" title='Adicionar contrato'  onclick="validarDatos(``, `I=<?= $var_add ?>`, `modalVentana`, `View/Contratos/ContratosModales.php`, event, 'ele')"><img src="img/icon/adds.png"/> ADICIONAR<br>CONTRATO</button>
        <button type='button' id='button' class="ele" title='Ayuda'  onclick="validarDatos(``, `I=<?= $var_ayu ?>`, `modalVentana`, `View/Contratos/ContratosModales.php`, event, 'ele')"><img src="img/icon/ayu.png"/> AYUDA<br>MODULO</button>
    </div>
